<?php
    ...
    $user_products_query="select name,email,contact,msg from contact";
...
<body>
  ...
  <div class="container">
                <table class="table table-bordered table-striped">
                    <tbody>
                        <tr>
                            <th>Sr.No.</th><th>Name</th><th>Email</th><th>Phone</th><th>Query</th><th></th>
                        <?php 
                        $user_products_result=mysqli_query($con,$user_products_query) or die(mysqli_error($con));
                        $no_of_user_products= mysqli_num_rows($user_products_result);
                        $counter=1;
                       while($row=mysqli_fetch_array($user_products_result)){ 
                         ?>
                        <tr>
              <th><?php echo $counter ?></th><th><?php echo $row['name']?></th><th><?php echo $row['email']?></th><th><?php echo $row['contact']?></th><th><?php echo $row['msg']?></th><th><form action="mailto:<?php echo $row['email']?>" method="post" enctype="text/plain"><input type="submit" value="Send Mail" class="btn btn-primary"></th>
                        </tr>
